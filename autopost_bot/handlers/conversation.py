...
def build_conversation_handler():
    return ConversationHandler(
        ...
        name="post_flow",
        persistent=False,
        # per_*: по умолчанию per_user=True, per_chat=True (одна беседа на юзера в чате).
        # per_message=True нельзя: при нём все хендлеры должны быть CallbackQueryHandler;
        # у нас entry_points и apply_edit — MessageHandler. См. PTB FAQ: per_* settings in ConversationHandler.
    )
